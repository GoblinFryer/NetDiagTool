...
$users = get-adgroupMember -identity "ProxyDenyFR" | select-object -ExpandProperty name

# Ajouter un gestionnaire d'événement au clic du bouton
$button.Add_Click({

    $textBox.clear()
    $script:users = get-adgroupMember -identity "ProxyDenyFR" | select-object -ExpandProperty name

    $computer = $textBoxComputer.Text
    # Exécuter la commande à distance et récupérer le résultat
    $result = Invoke-Command -ComputerName $computer -ScriptBlock {

        param($script:users,$computer)

        $profiles = Get-ChildItem "\\$computer\c$\users\" | Select-Object -ExpandProperty Name

        $Pdtext = "Proxy Deny not detected"  # Valeur par défaut

        foreach ($profile in $profiles) {
            if ($profile -in $script:users) {  # Vérification correcte
                $Pdtext = "Proxy Deny detected ($profile)"
                Write-Host "PD detected: $profile"
                break  # Sortir immédiatement si un utilisateur est trouvé
            }
        }
        ...
        $networkAdaptersConfig = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled -eq $true -and $_.DNSDomain -eq "global.scd.scania.com" }
        ...
        $intranetPing = (Test-Connection "francoprma" -Count 1).ResponseTime
        ...
        $resultText += "`r`n■ PROXY DENY`r`n"
        $resultText += "`r`n$PDtext`r`n"
        $resultText += "`r`n■ LATENCY`r`n"
        $resultText += "`r`nINTRANET PING(ms) | FRANCOPRMA : $intranetPing ms`r`nINTERNET PING(ms) | GOOGLE.COM : $internetPing ms`r`n"
